...
## Handling of Secrets
Porchlight never stores credentials in the repository. The Minds Builder API key
lives only in a gitignored `.env` (or `~/.config/`), and the app runs fully in
MOCK mode without any key.
